...
var KTDatatablesButtons = function () {
    ...
    var initDatatable = function () {
        let url     = `${HOST_URL}/measurement/get-imaq-all`;
        let columns = [
            {   //ID
                targets   : 0,
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                        <button type="button" data-id="${row.id}" class="btn btn-icon btn-light-warning update_medida">
                        <i class="bi bi-pencil"></i>
                        </button>
                        <button type="button" data-id="${row.id}" data-name="${row.name}" class="btn btn-icon btn-light-danger delete_medida">
    ...
    // Delete
    let deleteMedida = function () {
        $(document).on('click','.delete_medida',function(){
            let id   = $(this).data('id');
            let name = $(this).data('name');
            Swal.fire({
                text: `¿Estas seguro de querer eliminar la unidad de medida ${name}?`,
                icon: "warning",
    ...
    //Update
    let updateMedida = function () {
        $(document).on('click','.update_medida',function(){
            let id = $(this).data('id');
            $.ajax({
                url         : `/medida-imaq/${id}/edit`,
                dataType    : 'json',
                contentType : false,
                processData : false,
                type        : 'GET',
