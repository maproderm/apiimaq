...
var KTDatatablesButtons = function () {
    ...
    var initDatatable = function () {
        let url     = `${HOST_URL}/categorias/get-maproderm-all`;
        let columns = [
            {   //ID
                targets   : 0,
...

            {
                ...
                render: function (data, type, row) {
                    return `
                        <button type="button" data-id="${row.id}" class="btn btn-icon btn-light-warning update_category">
                        <i class="bi bi-pencil"></i>
                        </button>
                        <button type="button" data-id="${row.id}" data-name="${row.name}" class="btn btn-icon btn-light-danger delete_category">
    ...
    let deleteCategory = function () {
        $(document).on('click','.delete_category',function(){
            ...
            Swal.fire({
                text: `¿Estas seguro de querer eliminar la categoria ${name}?`,
                icon: "warning",
    ...
    let updateCategory = function () {
        $(document).on('click','.update_category',function(){
            ...
            $.ajax({
                ...
                if(!response.error){
                    $('#edit_category_mpd_modal').empty();
                    $('#edit_category_mpd_modal').append(response.render);
                    $('#kt_modal_update_category_mpd').modal('show');
                } else {
